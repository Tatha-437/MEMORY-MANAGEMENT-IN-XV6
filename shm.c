...
//you write this
	...
	int i = 0;
        ...
        if(flag == -1) //id not found in shm_table, let's find a suitable spot 'flag'
        {
		i = 0;	
		for(i = 0; i < 64; i++)
		{
			if(shm_table.shm_pages[i].id == 0) //this page is empty so this works
			{
			  flag = i;
        ...
        else	//id found, it is equal to 'flag'
        {
...
//you write this too!
	...
	for(i = 0; i < 64; i++)
	{
	  if(id == shm_table.shm_pages[i].id)
	  {
		//shm_table.shm_pages[i].refcnt -= 1;  not yet
